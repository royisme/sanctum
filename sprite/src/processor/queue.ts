...
export async function writeToQueue(env: Env, message: QueueMessage): Promise<void> {
...
export async function readQueue(env: Env): Promise<Array<{ key: string; data: QueueMessage }>> {
  ...
  return messages.filter((m) => m !== null) as Array<{ key: string; data: QueueMessage }>
}

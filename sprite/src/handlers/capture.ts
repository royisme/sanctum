...
export function createCaptureHandler() {
  return async (ctx: Context): Promise<CaptureEvent | null> => {
    ...
    if (message.entities) {
      ...
      if (urlEntity) {
        ...
        if ('url' in urlEntity && urlEntity.url) {
          ...
          sourceUrl = rawText.substring(urlEntity.offset, urlEntity.offset + urlEntity.length)
        }
      }
    }
    ...
    if (!sourceUrl && message.caption_entities) {
      const urlEntity = message.caption_entities.find((e) => e.type === 'url' || e.type === 'text_link')
      if (urlEntity) {
        sourceType = 'url'
        if ('url' in urlEntity && urlEntity.url) {
          sourceUrl = urlEntity.url
          ...
          sourceUrl = caption.substring(urlEntity.offset, urlEntity.offset + urlEntity.length)
        }
      }
    }
